...
CREATE PROCEDURE UpdateCommCounts( time1 BIGINT, time2 BIGINT)
  BEGIN
    ...
    DECLARE tpCursor CURSOR FOR SELECT sender, recipient FROM Comm WHERE time1 <= time AND time < time2;
    DECLARE CONTINUE HANDLER FOR NOT FOUND SET done = TRUE;

    OPEN tpCursor;

    tpLoop: LOOP
      FETCH tpCursor INTO tSender, tRecipient ;

      IF done THEN
        LEAVE tpLoop;
      END IF;

      IF tSender < tRecipient THEN
        UPDATE TraderTraderEdge
        SET comms = comms + 1,
          commWgt = commWgt * 1.1
        WHERE trader1 = tSender AND trader2 = tRecipient;

      ELSEIF tRecipient < tSender THEN
        UPDATE TraderTraderEdge
        SET comms = comms + 1,
          commWgt = commWgt * 1.1
...
CREATE PROCEDURE UpdateTradeCounts( time1 BIGINT, time2 BIGINT)
  BEGIN
    ...
    DECLARE tpCursor CURSOR FOR
      SELECT buyer, seller, symbol, sector, count(*) as count
      FROM Trade WHERE time1 <= time AND time < time2
    ...
    tpLoop: LOOP
    FETCH tpCursor INTO tBuyer, tSeller,tSymbol,tSector,tCount ;

    IF done THEN
      LEAVE tpLoop;
    END IF;

      IF tBuyer < tSeller THEN
        UPDATE TraderTraderEdge
          SET tradeWgt = tradeWgt * 1.1
        WHERE trader1 = tBuyer AND trader2 = tSeller;

        ELSEIF tSeller < tBuyer THEN
          UPDATE TraderTraderEdge
          SET tradeWgt = tradeWgt * 1.1
